...
**Eval alignment:** Hints **MUST** use the **same** normalization as **`:`** evaluation for that field. For **`otag`** / **`atag`**, candidate matching for the digest uses the **same boundary-aligned rule** as eval (see **Tag fields (`otag` / `atag`) alignment digest** below).

**Tag fields (`otag` / `atag`) alignment digest:** [Spec 174](174-otag-atag-prefix-query-semantics.md) **`:`** evaluation unions every wire key whose **`normKey`** matches the normalized user prefix **`u`** at the **start of the slug** or **after a hyphen**. For the breakdown digest, for **non-empty** **`u`**, enumerate the **same** valid start indices **`i`** as eval ( **`i === 0`** or **`normKey[i - 1] === '-'`** ) where **`normKey.slice(i).startsWith(u)`**, and define **remainder** **`R = normKey.slice(i + u.length)`** at each such alignment.

- **Single-completion** **`(`** *suffix* **`)`** (no `|`): when **exactly one** distinct **`normKey`** in the vocabulary has at least one matching alignment, and there is **exactly one** distinct **non-empty** **`R`** across those alignments. The hint is **`(`** + **`R`** + **`)`**. **Omit** the hint if **every** alignment yields **`R === ""`** (segment already complete; e.g. **`trigger`** on **`death-trigger`**). **Omit** if the sole matching **`normKey === u`** (typed value already equals the full tag key).
- **Multi-branch** **`(|)`** with range collapsing: when **two or more** distinct **`normKey`** values have a matching alignment, **or** one **`normKey`** has **multiple** alignments whose non-empty **`R`** values yield **more than one** distinct first code unit (or mixed empty / non-empty—see below). Collect branch tokens from the **first code unit** of each **non-empty** **`R`**; **dedupe**, **sort**, **collapse** per **Range collapsing**. If **any** alignment has **`R === ""`** while **another** alignment (same or different key) has **`R` non-empty**, include **`=`** as a branch (same role as rule 3 for other fields).
- **Empty** trimmed value (`otag:`, `atag:`): unchanged—first-character digest over **all** non-empty normalized candidates.

**Rendering exception:** For fields other than **`otag`** / **`atag`**, **Rendering (non-empty prefix)** rules 1–3 use **`c.slice(prefix.length)`** from **string start**. For **`otag`** / **`atag`**, use the **alignment digest** rules above (remainder **`R`** at each boundary, not only left-anchored **`normKey.slice(u.length)`**).
...
These are the fields where **evaluation** on **`:`** applies normalized prefix discovery over the vocabulary (or per-row strings, or the **`in:`** union of namespaces below) and **ORs** matches—**except** **`otag`** / **`atag`**, which use **boundary-aligned** matching per Spec 174. Breakdown hints **MUST** eventually support **every** row (implementation may land in one PR or be phased; the spec is the full target set).
...
| `otag` | `otag`, `function` → `otag`, `oracletag` → `otag` | Oracle tag labels (loaded index); hints use Spec 174 boundary-aligned **`:`** matching | 174 |
| `atag` | `atag`, `art` → `atag` | Illustration tag labels (loaded index); hints use Spec 174 boundary-aligned **`:`** matching | 174 |
...
**Normalization:** For fields other than **`otag`** / **`atag`**, hints use the same **`normalizeForResolution`** as evaluation (Spec 103) on both the user value and each candidate string. For **`otag`** / **`atag`**, hints use **`normalizeForTagResolution`** and the same **boundary-aligned** candidate filter as Spec 174 **`:`** eval.
...
- **Non-empty trimmed value:** Show a hint when the digest can be formed per **Content** (for **`otag`** / **`atag`**, see **Tag fields (`otag` / `atag`) alignment digest**—omit only when rules there say to omit). Use **single-completion** vs **multi-branch** rendering below.
...
**Range collapsing (normative):** When a multi-branch hint contains **three or more contiguous** single-character branches in ASCII order (letters `a–z` or digits `0–9`), they **MUST** be collapsed into a **double-dot range**: **`X..Y`** (two **U+002E FULL STOP** characters) where `X` is the first and `Y` the last character in the run. Runs of length 1 or 2 remain individual pipe-separated characters. Letter ranges and digit ranges are treated independently (a run never spans from `9` to `a`). Examples:
...
**Why `..` not `-`:** For most fields, branch tokens are single ASCII letters or digits. For **`otag`** / **`atag`**, normalized candidates may contain **`-`** (U+002D HYPHEN-MINUS) per Spec 174, so **`-`** must **not** denote a character range. **`|`** remains the multi-branch separator. The `=` exact-prefix marker (rendering rule 3) is not part of any range and appears as its own branch when present.
...
4. Hints use the **same** normalization as **`:`** evaluation: Spec 103 **`normalizeForResolution`** for most in-scope fields; [Spec 174](174-otag-atag-prefix-query-semantics.md) **`normalizeForTagResolution`** for **`otag`** / **`atag`**. For **`otag`** / **`atag`**, digest content for non-empty prefixes follows **Tag fields (`otag` / `atag`) alignment digest**; evaluation remains the full boundary-aligned union per Spec 174.
5. For **`otag:`** / **`atag:`** with a prefix that matches **only** at interior segment boundaries (e.g. **`on-`** matching **`cast-on-resolution`**), the breakdown shows a **non-empty** `prefixBranchHint` when at least one alignment yields a **non-empty** remainder **`R`** (e.g. multi-branch first characters including **`r`** from **`resolution`** when multiple keys match).
...
- **`otag:` / `atag:`** hints **MUST** enumerate boundary alignments with the **same** start-index loop as [Spec 174](174-otag-atag-prefix-query-semantics.md) / `matchesBoundaryAlignedPrefix` (shared helper next to that function in `shared/src/normalize.ts`) so eval and hints cannot drift.
