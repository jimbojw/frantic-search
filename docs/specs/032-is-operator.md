...
| `is:partner` | **Superset** of Scryfall’s `is:partner` (intentionally may match extra cards). Faces whose type line contains `Saga` never match (no saga-partner assumption; revisit if WotC prints one). If the type line contains `Creature`, it must also contain `Legendary` (excludes Battlebond-style nonlegendary `Partner with` creatures that Scryfall does not treat as `is:partner`). A face matches if **any** of: Scryfall `keywords` lists `Partner` or `Partner with …` for that oracle (`keywords_index`); oracle text (after reminder strip) has a standalone `Partner` line or a `Partner with` line; type line contains `Background`; oracle contains `choose a background`, `doctor's companion`, or `commander creatures you own`; or type line contains `Time Lord Doctor`. |
...
- **Printing-level attributes:** `is:foil`, `is:etched`, `is:nonfoil`, `is:fullart`, `is:textless`, `is:promo`, `is:digital`, `is:oversized`. (These are implemented as printing-domain keywords per Spec 047; `is:glossy`, `is:rebalanced` via `promo_types`; `is:alchemy` uses the same promo bit but ETL also sets it from `set_type: "alchemy"` — Specs 046 and 047.)
