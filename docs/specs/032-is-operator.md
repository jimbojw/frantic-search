...
| `is:commander` | Front face type line contains `Legendary` AND (`Creature` or `Vehicle` or `Background`), OR oracle text contains `"can be your commander"` or `"spell commander"`, OR hardcoded exception (e.g. Grist); excludes tokens, meld results, and cards banned in Commander (Scryfall parity, Issue #148, #149) |
