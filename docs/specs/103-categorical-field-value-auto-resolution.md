...
**`otag` / `atag` (exception):** Do **not** use **`normalizeForResolution`** for tag labels or tag field values in resolution or evaluation. Use **`normalizeForTagResolution`** from [Spec 174](174-otag-atag-prefix-query-semantics.md): same NFD/diacritic/lowercase pipeline, but keep **`[a-z0-9-]`** (hyphens preserved). Stripping hyphens broke prefix typing ([GitHub #253](https://github.com/jimbojw/frantic-search/issues/253)). **`resolveForField("otag", …)`** / **`resolveForField("atag", …)`** normalize with **`normalizeForTagResolution`** and match candidates when **`normalizeForTagResolution(candidate).startsWith(normalizeForTagResolution(typed))`** for unique-prefix resolution—**full-string prefix** on the tag slug, not the boundary-aligned **`:`** eval rule (Spec 174 separates evaluation from canonicalize).
...
| `otag` | Oracle tag vocabulary (when loaded); normalize with **`normalizeForTagResolution`** ([Spec 174](174-otag-atag-prefix-query-semantics.md)) | Runtime |
| `atag`, `art` | Illustration tag vocabulary (when loaded); same tag normalization as **`otag`** | Runtime |
...
**Exception — `game` query evaluation (Spec 068 / Spec 182):** The printing-domain **`game`** leaf does **not** use `resolveForField` before semantic matching for **`:`** / **`=`** / **`!=`**. The AST **operator** selects **prefix** vs **exact** vs **`!=`** (negation of **`=`** exact mask only) on normalized keys of **`GAME_NAMES`**, same pattern as **`frame:`**. A **non-empty** trimmed value with **no** key matching under the active operator yields **`unknown game "…"`** with passthrough (Spec 039). Empty **`game:`** / **`game=`** / **`game!=`** are neutral (all printings). **`resolveForField("game", …)`** remains for **canonicalize** and other non-eval consumers.

**Exception — `rarity` / `r` query evaluation (Spec 047 / Spec 182):** The printing-domain **`rarity`** leaf does **not** use `resolveForField` before semantic matching for **`:`** / **`=`** / **`!=`**. The AST **operator** selects **prefix** vs **exact** vs **`!=`** (negation of **`=`** exact mask only) on normalized keys of **`RARITY_NAMES`**, same pattern as **`frame:`**. **Comparison operators** (**`>`**, **`>=`**, **`<`**, **`<=`**) still call **`resolveForField("rarity", …)`** (or **`r`**) for a single anchor tier, then apply ordinal **`buildRarityMask`** logic (Spec 047). A **non-empty** trimmed value with **no** key matching under **`:`** / **`=`** / **`!=`** yields **`unknown rarity "…"`** with passthrough (Spec 039). Empty **`rarity:`** / **`r=`** / **`r!=`** (and alias **`r:`**) are neutral (all printings). **`resolveForField("rarity", …)`** remains for **canonicalize** and comparison RHS.
...
**Exception — `otag` / `atag` query evaluation (Spec 174):** Face-domain **`otag:`** (and aliases **`function`**, **`oracletag`**) and printing-domain **`atag:`** / **`art:`** leaves do **not** use `resolveForField` before matching. The AST **operator** selects **boundary-aligned prefix** (**`:`**) vs **exact** (**`=`**) vs **`!=`** (negation of **`=`** exact mask only) on **precomputed** **`normKey`** strings from **`normalizeForTagResolution`** per wire key (Spec 174). A **non-empty** value that matches **no** key under the active operator yields **`unknown oracle tag "…"`** or **`unknown illustration tag "…"`** with **passthrough** (Spec 039), same family as **`kw:`** / **`set:`**. **`resolveForField("otag", …)`** and **`resolveForField("atag", …)`** remain for **canonicalize** and other non-eval consumers that need unique-prefix resolution to a single label using **tag** normalization (§2), not global **`normalizeForResolution`**.

**Exception — `kw` / `keyword` query evaluation (Spec 176):** Face-domain **`kw:`** / **`keyword:`** / **`kw=`** / **`keyword=`** leaves do **not** use `resolveForField` before matching. The AST **operator** selects **prefix** (**`:`**) vs **exact** (**`=`**) on normalized index keys; matching keys **union** face indices. A **non-empty** value that matches **no** key under that operator yields **`unknown keyword "…"`** with **passthrough** (Spec 039), same family as **`set:`** / **`set_type:`** (Spec 047 / 179) and **`otag:`** / **`atag:`** (Spec 174). **`resolveForField("kw", …)`** and **`resolveForField("keyword", …)`** remain for **canonicalize** and other non-eval consumers that need unique-prefix resolution to a single keyword string.
...
| `shared/src/search/eval-printing.ts` | **`in`** lookup: `resolveForField` where applicable per Spec 072. **`set`** / **`set_type`** use operator-selected prefix or exact on precomputed normalized row strings (no `resolveForField` in the leaf; Specs 047 / 179). **`frame`**, **`game`**, and **`rarity`** (**`:`** / **`=`** / **`!=`**) use operator-selected prefix or exact on normalized vocabulary keys (no `resolveForField` in the leaf; Specs 047 / 068 / 182). **`rarity`** **comparison operators** use **`resolveForField`** then **`buildRarityMask`** (Spec 047). |
...
6. **`rarity:r`** with **`:`** matches printings at **rare** (prefix union over **`RARITY_NAMES`** keys **`r`** and **`rare`** ORs the same bit). **`rarity=r`** matches printings when the normalized key **`r`** equals **`u`** (exact), not **`rare`** unless the user types **`rare`**.
7. **`game:a`** with **`:`** ORs **arena** and **astral** (every **`GAME_NAMES`** key whose normalized form starts with **`a`**). **`game=arena`** matches Arena only. **`resolveForField("game", "ar")`** may still resolve to **`arena`** for canonicalize when unique.
...
14. **`otag:`** / **`atag:`** evaluation uses Spec 174 (**`:`** boundary-aligned prefix union, **`=`** exact on **`normalizeForTagResolution`**, **`!=`** negates **`=`**); non-empty no match → **`unknown oracle tag`** / **`unknown illustration tag`** (passthrough).
...
20. **`game:`** / **`game=`** / **`game!=`** evaluation (Spec 068 / 182) does **not** use **`resolveForField`** for semantic matching on **`:`** / **`=`** / **`!=`**; **`:`** ORs **`GAME_NAMES`** bits for prefix matches; **`=`** / **`!=`** use exact mask rules per Spec 182. Non-empty no match → **`unknown game "…"`** (passthrough). Empty operators neutral. **`resolveForField("game", …)`** remains for canonicalize when unique.
21. **`rarity:`** / **`r=`** / **`r!=`** / **`r:`** aliases evaluation (Spec 047 / 182) does **not** use **`resolveForField`** for **`:`** / **`=`** / **`!=`**; comparison operators still use **`resolveForField`**. Non-empty no match → **`unknown rarity "…"`** (passthrough). Empty operators neutral. **`resolveForField("rarity", …)`** remains for canonicalize when unique.
