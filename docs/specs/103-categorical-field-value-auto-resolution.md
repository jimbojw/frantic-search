...
**Exception — `set` query evaluation (Spec 047, [issue #234](https://github.com/jimbojw/frantic-search/issues/234)):** The printing-domain **`set`** leaf does **not** use `resolveForField` before matching. It applies **prefix matching** on normalized set codes per printing row. Short or ambiguous tokens (e.g. `set:u`) intentionally match **many** printings for discovery; they are **not** collapsed to a single code when that code would be uniquely resolvable from `knownSetCodes`. A **non-empty** trimmed value with **no** printing whose code prefix-matches yields **`unknown set "…"`** with passthrough (Spec 039), not a silent zero-hit leaf. **`resolveForField("set", …)`** remains used for **canonicalize** (`toScryfallQuery`) and any other non-eval sites that need unique-prefix resolution to a full code.
...
10. `set:xyz` with no printing whose set code prefix-matches yields **`unknown set "xyz"`** with passthrough (Spec 047 / [issue #234](https://github.com/jimbojw/frantic-search/issues/234) prefix semantics; error on no match as of 047 changelog 2026-04-02).
