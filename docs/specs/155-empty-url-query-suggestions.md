...
5. **No change** to landing with **no** `q` parameter: no worker search, hero-style home behavior unchanged. Programmatic desktop auto-focus still must **not** append `q=` by itself. A **user gesture** on the search input sets engagement (including **pointer down** on the textarea when focus was already established by auto-focus—see Spec 060); only then may the URL gain empty `?q=` and trigger this spec’s worker path.
...
- 2026-03-23: Spec 060 `pointerdown` engagement ensures users can reach empty-URL starter suggestions after desktop auto-focus by clicking the textarea without blurring first.
