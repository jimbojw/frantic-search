...
- 2026-03-07: When `my:` is in the query, aggregation counts use list entry count. The list ID must match the queried list: `my:list`/`my:default` → default; `my:trash` → trash. Added `getMyListIdFromBreakdown()` to extract the list ID from the effective query.
