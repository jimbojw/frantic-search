...
| Error Type | Fix |
|------------|-----|
| **Unknown set** | Remove set/collector; resolve by name only. Card must resolve by name. |
...
**Unknown set:** Reconstruct the line without the `(SET)` or `(SET:num)` token. Preserve quantity, name, foil markers, tags, etc. The card must resolve by name (we already failed on set lookup, so we know the card was found by name earlier).
