...
- **Existing tests.** All existing parser tests construct expected AST nodes without `span`. Since `span` is optional, existing deep-equality assertions pass without modification — extra properties on actual nodes do not cause mismatches. New span-specific tests are added separately.
...
| `shared/src/search/lexer.test.ts` | Add tests verifying token spans for representative inputs |
...
| `shared/src/search/parser.test.ts` | Add tests verifying AST node spans for representative inputs |
...
All existing parser tests pass without modification. Since `span` is optional, tests that assert on AST structure without checking spans continue to work.
...
7. Existing parser tests pass without modification (spans are optional, not checked by existing assertions).
