...
```
Browser parses HTML
  → index.css (from <link> in head) and index.tsx (script) load in parallel
...
- 2026-03-20: Hero-style loading fallback: Suspense fallback now shows the ThumbHash hero (same position as real hero), gentle opacity pulse (`animate-loading-pulse`, 2.5s cycle), and "Loading search…" where the title/subtitle go. Shared `HEADER_ART_BLUR` moved to `hero-constants.ts`. `prefers-reduced-motion` disables the pulse.
- 2026-03-20: Moved index.css to HTML `<link>` so Tailwind loads in parallel with the script rather than after; removed CSS import from index.tsx.
- 2026-03-21: Fixed layout shift when App loads. AppShell's header had `mx-auto max-w-4xl px-4 pt-[...]`; the portaled header adds the same, causing double padding (bar moved down, became narrower). Moved those classes onto `#shell-minimal-bar` so minimal bar and portaled header share identical layout. Outer header is now an unstyled wrapper.
