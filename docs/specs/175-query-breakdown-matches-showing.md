# Spec 175: Query Breakdown MATCHES vs SHOWING (Default Inclusion Filter)

**Status:** Implemented

**References:** [GitHub #211](https://github.com/jimbojw/frantic-search/issues/211)

**Depends on:** [Spec 178](178-default-search-inclusion-filter.md) (default inclusion filter, pre-filter counts; draft, supersedes Spec 057), [Spec 079](079-consolidated-query-accordion.md) (unified accordion footer), [Spec 151](151-suggestion-system.md) (`include-extras` suggestion)

**Extends:** Spec 079 (summary footer rows), Spec 178 / Spec 057 (worker `result` fields)

## Goal

The unified query breakdown accordion footer labels the live row **MATCHES** but, today, shows **post–default-inclusion-filter** card/print counts—the same numbers as the visible result list. Breakdown chips already show **pre-filter** semantic match counts from the evaluator, so users can see a large chip total and a tiny **MATCHES** footer with no explanation.

Surface **MATCHES** (full match count after pinned∩live combination, before the default inclusion filter per Spec 178) and **SHOWING** (what the list actually uses) when the filter hides at least one result and there is a non-empty result list. The **`include:extras` rewrite** remains in **SuggestionList** (empty state and non-empty rider per Spec 178 / Spec 057 / 151), not inline on the accordion footer, so the footer stays a simple vertical stack of label + counts.

## Definitions

- **Pre-filter (MATCHES source):** Same semantics as `indicesBeforeDefaultFilter` and `printingIndicesBeforeDefaultFilter` in `runSearch` (Spec 178; renamed from Spec 057 `indicesIncludingExtras` / `printingIndicesIncludingExtras`): populated only when `include:extras` is absent and the default inclusion filter removed at least one card or printing from the combined pinned+live candidate set.
...
1. **Split (MATCHES + SHOWING)** when **all** are true:
   ...
   - The default inclusion filter hid at least one **card** or **printing** (same condition as the Spec 178 / Spec 057 non-empty `include:extras` rider: `hiddenCards > 0 || hiddenPrintings > 0`, using pre/post counts from the worker).
...
2. **Single live row (MATCHES only)** when split does not apply:
   ...
   - **Zero post-filter with hidden pre-filter matches:** **MATCHES** shows **pre-filter** card count (and printing count when `printingIndicesBeforeDefaultFilter` is present and printing counts are relevant), so the footer aligns with breakdown chips. **Do not** add a **SHOWING** row (no redundant “0 cards”).
...
Non-empty and empty-state **`include-extras`** suggestions behave as in Spec 178 / Spec 057 / 151 (sky-panel **SuggestionList**). The **SHOWING** row’s **Learn more** link points at the user-facing **`include:extras`** reference doc, which documents **MATCHES** / **SHOWING** and when the split row appears.
...
Expose optional fields on the `type: 'result'` message (as already described in Spec 178 / Spec 057 prose; implementation aligns type + payload):
...
6. When split is shown, the Spec 178 / Spec 057 non-empty **`include-extras` rider** in **SuggestionList** still appears when the worker supplies that suggestion (unchanged from pre–accordion-chip behavior).

## Changes by layer

| Layer | Change |
|-------|--------|
| `shared/src/worker-protocol.ts` | Add optional `indicesBeforeDefaultFilter`, `printingIndicesBeforeDefaultFilter` to `result` (renamed from `indicesIncludingExtras` / `printingIndicesIncludingExtras` per Spec 178). |
...
- Update Spec 178 / Spec 057: reference Spec 175 for accordion UX; clarify `result` fields as canonical for UI.
