...
- 2026-02-28: When `unique:prints` bypasses dedup, each tile/row now shows its individual finish label ("Foil", "Nonfoil", or "Etched") and price rather than the aggregated group. Foil and etched tiles receive a visual shimmer overlay on the card image (animated holographic gradient via CSS `::after`) and a rainbow-gradient background on the metadata text bar in Images mode. The animation respects `prefers-reduced-motion`. This addresses the problem of visually identical adjacent tiles when the same `scryfall_id` appears for multiple finishes.
