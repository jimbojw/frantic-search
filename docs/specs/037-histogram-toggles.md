...
- 2026-02-26: Added `clearColorIdentity` function to `query-edit.ts` and a "Clear" button to the color identity histogram column. The button appears in the 8th row slot (below multicolor) when any CI filter is active, and removes all CI-related nodes from the query.
- 2026-02-27: The TERMS drawer (Spec 044) now reuses `toggleSimple`, `findFieldNode`, `removeNode`, and `parseBreakdown` from `query-edit.ts` for tri-state cycling chips on format and `is:` keywords. This fulfills the "future TERMS drawer controls" anticipated in the Out of Scope section.
