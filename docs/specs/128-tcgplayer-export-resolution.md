...
- **Card name format:** TCGPlayer format uses combined name for split cards (`Supply // Demand`); transform/modal DFCs use front-face-only (`Delver of Secrets`). `resolveCardName` checks `layout === "split"` to emit combined name when `frontFaceOnly` is true.
