...
- **Design note:** Future disambiguation specs (e.g. commander → name vs `f:` vs `is:commander`, regex oracle phrases) may add new suggestion ids and priority slots between these tiers without collapsing oracle (**20**) and tag bare-term chips (**21**).
