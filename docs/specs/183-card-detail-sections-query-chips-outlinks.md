...
| Keywords | Oracle (combined block for multi-face; single-face table) | **Always show.** Comma-separated list or italic **none** when empty | One `kw:` chip per keyword; **chips column empty** when there are no keywords |
