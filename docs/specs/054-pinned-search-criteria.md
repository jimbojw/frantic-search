...
- **PINNED lip:** Shows the card count and (when applicable) printing count for the pinned query in isolation. When the pinned query contains `unique:prints` or printing-level terms, the printing count is shown.
- **MATCHES lip:** Shows the card count and (when applicable) printing count for the intersection (pinned AND live).
