...
| **Positive `set:` / `s:` / `e:`** (`:` only) | Contributes **set-code prefix** widening. **Rule:** a `set` FIELD with operator **`:`** and **non-empty** trimmed value positively constrains when (a) the normalized user prefix is a **non-empty prefix** of the printing's **precomputed** normalized set code (same as `evalPrintingField` **`set:`** — e.g. `set:wc0` widens `wc01`, `set:w` widens codes starting with `w`), **and** (b) the node has an **even number** of `NOT` ancestors. Empty `set:` does not add a widener. |
| **Positive `set=` / `s=` / `e=`** | Contributes **set-code exact** widening. **Rule:** operator **`=`**, **non-empty** trimmed value, even `NOT` depth; printing widens when `normalizeForResolution(setCode) === normalizeForResolution(node.value)` (same as `evalPrintingField` **`set=`**). Empty `set=` does not add a widener (neutral eval). |
| **Positive `set_type:` / `st:`** (`:` only) | Contributes **set-type prefix** widening, **combined** with set-code widening into one per-printing **wide** flag. **Rule:** operator **`:`**, **non-empty** trimmed value, even `NOT` depth; printing widens when its **precomputed** normalized `set_type` **starts with** the normalized user prefix (same as `evalPrintingField` **`set_type:`** — e.g. `st:m` matches `masters` and `memorabilia`). Empty `set_type:` does not add a widener. |
| **Positive `set_type=` / `st=`** | Contributes **set-type exact** widening. **Rule:** operator **`=`**, **non-empty** trimmed value, even `NOT` depth; printing widens when normalized type **equals** normalized user value (same as `evalPrintingField` **`set_type=`**). Empty `set_type=` does not add a widener. |
| **`is:<extras-layout>`** | A positive `is` / `not` node with operator **`:`** or **`=`** whose **Spec 032** expanded keyword set **L** includes **any** member of the extras-layout set (`token`, `dfctoken` / `double_faced_token`, `art_series`, `vanguard`) sets `widenExtrasLayout`. **`is!=`** / **`not!=`** do **not** set this flag from the excluded keyword. For **each printing whose canonical face layout is in the extras-layout set**, the **per-printing omission gate** opens: passes **1–5** are skipped for that printing (same as printing-wide `set:` / `st:` widening for that row)—including memorabilia (**3a**), wholesale codes (**3b**), playtest, content-warning, and oversized. Does **not** widen normal-layout printings. Structural detection mirrors `includeExtras`: walk the AST; a node with an even number of `NOT` ancestors (0, 2, …) counts as positive. |
| **`is:content_warning`** | Disables the **content-warning** default omission pass for the query when a positive `is` / `not` node with operator **`:`** or **`=`** has **Spec 032** expanded set **L** including **`content_warning`**. **`!=`** excluded. Structural detection: even number of `NOT` ancestors (same rule as above). |
| **`is:playtest`** | Disables the **playtest** default omission pass for the query when a positive `is` / `not` node with operator **`:`** or **`=`** has expanded set **L** including **`playtest`**. **`!=`** excluded. Structural detection: even number of `NOT` ancestors (same rule as above). |
| **`is:oversized`** | Disables the **oversized** default omission pass for the query when a positive `is` / `not` node with operator **`:`** or **`=`** has expanded set **L** including **`oversized`**. **`!=`** excluded. Structural detection: even number of `NOT` ancestors (same rule as above). |
...
- **`positiveSetPrefixes`:** normalized non-empty prefixes from positive **`set:`** / **`s:`** / **`e:`** / **`edition:`** with operator **`:`** only (even `NOT` depth).
- **`positiveSetExact`:** normalized non-empty **exact** values from positive **`set=`** / **`s=`** / **`e=`** / **`edition=`** (even `NOT` depth).
- **`positiveSetTypePrefixes`:** normalized non-empty prefixes from positive **`set_type:`** / **`st:`** with operator **`:`** only (even `NOT` depth).
- **`positiveSetTypeExact`:** normalized non-empty exact values from positive **`set_type=`** / **`st=`** (even `NOT` depth).
- **Per-printing wide flag** (worker / CLI parity): `wide = setWide || typeWide` where `setWide` ⇔ some entry in **`positiveSetPrefixes`** is a **prefix** of the printing's **precomputed** normalized set code **or** some entry in **`positiveSetExact`** **equals** that normalized code; `typeWide` ⇔ analogous **prefix** / **exact** match against the printing's **precomputed** normalized set type. Pinned + live: merge all four arrays from pinned and live `EvalOutput` like today for set prefixes.
...
| Area | Action |
|------|--------|
| `shared` `EvalOutput` | Add `widenExtrasLayout`, …; keep `includeExtras`. **`positiveSetTypePrefixes`**, **`positiveSetExact`**, **`positiveSetTypeExact`** (Spec 178 / 047 / 179 operator split). |
| `shared` `evaluator.ts` | AST helpers; **`_collectPositiveSetPrefixes`** (`:` only), **`_collectPositiveSetExact`**, **`_collectPositiveSetTypePrefixes`** (`:` only), **`_collectPositiveSetTypeExact`**. |
...
9. Set-code widening: **`set:`** uses **prefix** matching consistent with `evalPrintingField` (`set:w` widens codes starting with `w`). **`set=`** uses **exact** normalized equality consistent with `evalPrintingField`.
9a. **Set-type widening:** **`set_type:`** / **`st:`** use normalized-prefix rules; **`set_type=`** / **`st=`** use normalized exact equality—aligned with evaluation.
...
11. Pinned + live query: **`positiveSetPrefixes`**, **`positiveSetExact`**, **`positiveSetTypePrefixes`**, and **`positiveSetTypeExact`** merge pairwise (union of lists when both queries contribute).
...
- 2026-04-04: **`is` / `not`** widener detection uses **Spec 032** operator-aware expansion (**`:`** or **`=`** only; **`!=`** does not widen from the value).
- 2026-04-02: **`is:` / `not:`** widener detection uses **Spec 032** expansion over the `is:` vocabulary (any expanded keyword in the widener set triggers the flag), not Spec 103 unique-prefix resolution only.
...
- Positive set **prefix** keys: `_collectPositiveSetPrefixes` (`set` / aliases, operator **`:`** only). Positive set **exact** keys: **`_collectPositiveSetExact`** (`operator === "="`). Positive set-type **prefix** keys: **`_collectPositiveSetTypePrefixes`** (`:` only). Positive set-type **exact** keys: **`_collectPositiveSetTypeExact`**. Collection is **AST-only**; bogus tokens still yield empty widening matches.
- Constants and helpers **`isMemorabiliaDefaultOmit`**, **`isSetTypeWidenedByPrefixes`**, **`isSetTypeWidenedByExact`** (or equivalent) live in `shared/src/search/default-filter.ts`. Per-printing widening compares against **`PrintingIndex`** precomputed normalized set code / set type strings (Spec 047 / 179 / 182).
- 2026-04-04: Widening split for **`set=`** / **`set_type=`** exact vs **`:`** prefix (aligned with Spec 047 / 179 operator split).
