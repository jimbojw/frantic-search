...
- **Sort comparators (Spec 059):** Do not allocate inside `Array.prototype.sort` comparison callbacks — no fresh objects, and no `localeCompare` calls that pass a new options object each time; whole-sort **O(n)** buffers and a single reused collator are fine. See `docs/specs/059-sort-directives.md` § Sort implementation → Allocation discipline.
