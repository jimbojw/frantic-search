...
interface DefaultCard {
  ...
  promo_types?: string[];
  /**
   * Scryfall set type on the printing. When `alchemy`, `encodePromoTypesFlags` sets the `alchemy`
   * promo bit for `is:alchemy` parity (Spec 046). When `funny`, `encodePrintingFlags` sets
   * `PrintingFlag.Unset` for `is:unset` parity (Spec 171).
   */
  set_type?: string;
